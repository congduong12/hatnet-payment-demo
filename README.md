...
This repository is in Harness v0 with the first VN Payment Learning Lab
application foundation added.
...
The current source spec is `SPEC.md`. Living product contracts are derived from
that file under `docs/product/`.
...
Use `SPEC.md` as input material, not the long-term operating plan. Future work
should update the smaller product docs, story packets, durable Harness records,
and decisions.
...
Smoke-check the API after `pnpm dev:api` starts:
