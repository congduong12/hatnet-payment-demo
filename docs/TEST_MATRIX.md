...
| US-006 | Checkout and currency foundation: immutable order snapshots, integer-safe USD-to-VND conversion, rounding, transactional cart close | yes | yes | no | yes | implemented | `pnpm --filter @hatnet/api test`; `pnpm --filter @hatnet/api check`; `pnpm --filter @hatnet/api build`; `docker compose up -d postgres`; `pnpm db:migrate`; PostgreSQL smoke verified `pro-plan` checkout creates a `249000` VND order snapshot and closes the active cart; public `POST /checkout`, points, payOS, and browser UI deferred; `docs/stories/high-risk/US-006-checkout-currency-foundation/` |
| US-007 | GitHub AI review bot: `/improve` command reviews PR diffs with OpenAI for hidden bugs, security vulnerabilities, anti-patterns, missing tests, and contract drift | yes | yes | no | no | in_progress | `node --check scripts/github-review-bot/review-pr.mjs`; `pnpm ai-review:self-test`; `pnpm ai-review:dry-run`; `pnpm check`; `pnpm test`; `pnpm build`; GitHub PR smoke pending until `OPENAI_API_KEY` and `OPENAI_MODEL` are configured; `docs/stories/high-risk/US-007-github-review-bot/` |
