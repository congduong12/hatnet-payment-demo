...
| US-007 | Codex GitHub review integration: `@codex review` and optional Automatic reviews apply repo guidance for hidden bugs, security vulnerabilities, anti-patterns, missing tests, and contract drift | yes | yes | no | yes | in_progress | `pnpm check`; `pnpm test`; `pnpm build`; GitHub PR smoke confirmed `Pull Request Checks / checks (pull_request)` passed; Codex Cloud `@codex review` smoke pending; `docs/stories/high-risk/US-007-github-review-bot/` |
